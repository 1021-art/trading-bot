...
"""
IDX500 SHORT-ONLY v50.1 (HS0.055 MDD25 tuned) (PF-max @ Multiple>=7.6x, Policy-fixed)
------------------------------------------------------------------------------------
* v50 FIX:
...
def risk_ramp(eq: float) -> float:
    """Equity-based risk ramp (no lookahead): protects early period (e.g., 2021) without muting later compounding."""
    if not RISK_RAMP:
        return 1.0
...
# reason: 1 hard, 2 soft(intra), 3 tp2, 4 time, 5 soft(close)
    ...
    for i in range(n):
        ...
        if (qty == 0.0) and (not exited_this_bar) and (t_i >= block_until_ns):
            ...
            if len(entry_times) >= capN:
            ...
            elif (bear == 1 and np.isfinite(ema50) and np.isfinite(ret1) and
                ...
                if (c1h < ema50) and (ret1 <= RET1_THR) and (r30p <= RET30_PREV_THR):
                    ...
                    ramp = risk_ramp(eq)
                    vol_scale = 0.92 if matrp >= 0.030 else 1.0
                    qty = -(eq * RISK_PCT * scale * ramp * vol_scale)/ stop_dist
